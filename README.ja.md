...
埋め込みモデル(torch / sentence-transformers)のロードはコールド起動時に50秒超かかることがあります。環境変数 `ENGRAM_PRELOAD` でロードのタイミングを選べます。

| 値 | 挙動 |
|---|---|
| `background`(既定) | MCP ハンドシェイクに即応答し、モデルは裏スレッドで読み込む。初回 `recall` はモデル準備完了まで待つ。 |
| `blocking` | ハンドシェイク前にモデルを読み込む。初回 `recall` は速いが、起動タイムアウトの短いクライアント(例: Claude Code の既定30秒)ではコールド起動時に接続が切られることがある。 |
| `off` | 先読みしない。初回ツール呼び出し時に遅延ロードする。 |

エージェント側で engram が断続的に不通と表示される場合は、`blocking` に上書きしていないか確認するか、クライアント側の MCP 起動タイムアウトを延長してください(Claude Code なら `MCP_TIMEOUT=120000`)。
